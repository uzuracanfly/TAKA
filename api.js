...
async function RunAPIMethods(ArgsData,request,response){
	try{
		...
		if (ip != '127.0.0.1'){
			if (("WhiteMethodList" in CONFIG.API) && (CONFIG.API["WhiteMethodList"].length > 0) && (CONFIG.API["WhiteMethodList"]).indexOf(ArgsData["function"]) == -1){
				response.write(JSON.stringify(false));
				response.end();
				return false;
			}
			if ((CONFIG.API["BlackMethodList"]).indexOf(ArgsData["function"]) > -1){
				response.write(JSON.stringify(false));
				response.end();
				return false;
			}
